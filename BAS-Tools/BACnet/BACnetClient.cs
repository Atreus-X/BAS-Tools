...
namespace System.IO.BACnet
{
    public class BacnetClient : IDisposable
    {
        ...
        private int m_transmit_timeout = 30000;
        ...
        private LinkedList<byte[]> m_segments = new LinkedList<byte[]>();
        ...
        public int Retries { get { return m_retries; } set { m_retries = Math.Max(1, value); } }
        ...
        {
            m_client.Start();
            m_client.MessageRecieved += new MessageRecievedHandler(OnReceive);
        ...
        protected void ProcessConfirmedServiceRequest(BacnetAddress adr, BacnetPduTypes type, BacnetConfirmedServices service, BacnetMaxSegments max_segments, BacnetMaxAdpu max_adpu, byte invoke_id, byte[] buffer, int offset, int length)
        {
            try
            {
                ...
                else if (service == BacnetConfirmedServices.SERVICE_CONFIRMED_EVENT_NOTIFICATION && OnEventNotify != null)
                {
        ...
        protected void ProcessUnconfirmedServiceRequest(BacnetAddress adr, BacnetPduTypes type, BacnetUnconfirmedServices service, byte[] buffer, int offset, int length)
        {
            try
            {
                ...
                {
                    GlobalLogger.Log("I-Am packet identified. Attempting to decode...");
        ...
        private void ProcessSegment(BacnetAddress adr, BacnetPduTypes type, BacnetConfirmedServices service, byte invoke_id, BacnetMaxSegments max_segments, BacnetMaxAdpu max_adpu, bool server, byte sequence_number, byte proposed_window_number, byte[] buffer, int offset, int length)
        {
            ...
            if (!more_follows) m_last_sequence_number = 0;
        ...
        private void OnReceive(IBacnetTransport sender, byte[] buffer, int offset, int msg_length, BacnetAddress remote_address)
        {
            try
            {
        ...
        public int GetMaxApdu()
        {
            ...
            const int max_npdu_header_length = 4;
        ...
        public bool ReadRangeRequest(BacnetAddress adr, BacnetObjectId object_id, uint idxBegin, ref uint Quantity, out byte[] Range, byte invoke_id = 0)
        {
            ...
            using (BacnetAsyncResult result = (BacnetAsyncResult)BeginReadRangeRequest(adr, object_id, idxBegin, Quantity, true, invoke_id))
            {
                for (int r = 0; r < m_retries; r++)
                {
                    ...
                    {
                        Exception ex;
                        EndReadRangeRequest(result, out Range, out Quantity, out ex);
                        if (ex != null) throw ex;
                        else return true;
                    }
                    if (r < (m_retries - 1))
        ...
        private void HandleSegmentationResponse(BacnetAddress adr, byte invoke_id, Segmentation segmentation, Action<BacnetClient.Segmentation> transmit)
        {
            ...
            System.Threading.ThreadPool.QueueUserWorkItem((o) =>
            {
                ...
                Transport.MaxInfoFrames = segmentation.window_size;
                while (true)
                {
                    ...
                    {
                        byte current_number = segmentation.sequence_number;
                        WaitForSegmentAck(adr, invoke_id, segmentation, 0);
                        if (segmentation.sequence_number != current_number)
                        {
                            Trace.WriteLine("Oh, a retransmit", null);
                            more_follows = true;
        ...
        private void SendComplexAck(BacnetAddress adr, byte invoke_id, Segmentation segmentation, BacnetConfirmedServices service, Action<EncodeBuffer> apdu_content_encode)
        {
            ...
            if (EncodeSegment(adr, invoke_id, segmentation, service, out buffer, apdu_content_encode))
            {
                ...
                {
                    if (segmentation.max_segments != 0xFF && segmentation.buffer.offset > (segmentation.max_segments * (GetMaxApdu() - 5)))
                    {
    ...
    public static class Bvlc
    {
        ...
        public static int Decode(byte[] buffer, int offset, out BacnetBvlcFunctions function, out int msg_length, ref BacnetAddress remote_address)
        {
            function = 0;
            msg_length = 0;
